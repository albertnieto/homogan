...
def train(g_model, d_model, gan_model, dataset, latent_dim=100, n_epochs=100, n_batch=128):
    ...
    for i in range(n_epochs):
        ...
        for image_batch in dataset:
            ...
            X_real = image_batch
            y_real = tf.ones(image_batch[0].shape[0],1)
            # update discriminator model weights
            d_loss1, _ = d_model.train_on_batch(X_real, y_real)
            # generate 'fake' examples
            X_fake, y_fake = gan.generate_fake_samples(g_model, latent_dim, image_batch[0].shape[0])
            # update discriminator model weights
            d_loss2, _ = d_model.train_on_batch(X_fake, y_fake)
            # prepare points in latent space as input for the generator
            X_gan = gan.generate_latent_points(latent_dim, image_batch[0].shape[0])
            # create inverted labels for the fake samples
            y_gan = np.ones((image_batch[0].shape[0], 1))
            # update the generator via the discriminator's error
            g_loss = gan_model.train_on_batch(X_gan, y_gan)

            if x % 100 == 0:
                print(f"Batch: {x}/{STEPS_PER_EPOCH}")
            if x % 10 == 0:
                with writer.as_default():
...
with tf.device('/device:GPU:0'):
    train(generator, discriminator, theGan, training_dataset, 256, 100,BATCH_SIZE)
